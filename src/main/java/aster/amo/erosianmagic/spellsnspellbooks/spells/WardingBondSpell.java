...
@AutoSpellConfig
public class WardingBondSpell extends AbstractSpell {
    private final ResourceLocation spellId = new ResourceLocation(ErosianMagic.MODID, "warding_bond");
        ...
        if(playerMagicData.getAdditionalCastData() instanceof CastMultiTargetingData multiTargetingData) {
            multiTargetingData.getTargets().forEach(target -> {
                ...
                if(Utils.shouldHealEntity(entity, targetEntity) && targetEntity != null) {
                    targetEntity.addEffect(new MobEffectInstance(MobEffectRegistry.WARDING_BOND.get(), duration, 0, false, false, true));
                }
            });
            LivingEntity mainTarget = (LivingEntity) ((ServerLevel)level).getEntity(multiTargetingData.getMainTarget());
            if(mainTarget != null && Utils.shouldHealEntity(entity, mainTarget)) {
