...
@AutoSpellConfig
public class InflictWoundsSpell extends AbstractSpell {
    private final ResourceLocation spellId = new ResourceLocation(ErosianMagic.MODID, "inflict_wounds");
    ...
    public boolean checkPreCastConditions(Level level, int spellLevel, LivingEntity entity, MagicData playerMagicData) {
        return preCastTargetHelper(level, entity, playerMagicData, this, 4, .35f);
    }
        ...
        if (playerMagicData.getAdditionalCastData() instanceof CastTargetingData castTargetingData) {
            ...
            if (target != null) {
                ...
                for(int i = 0; i < spellLevel * 5; i++) {
                    ((ServerLevel) level).sendParticles(ParticleRegistry.BLOOD_PARTICLE.get(), target.getX(), target.getY() + 1, target.getZ(), 3, 0.1, 0.25, 0.1, 0.1);
