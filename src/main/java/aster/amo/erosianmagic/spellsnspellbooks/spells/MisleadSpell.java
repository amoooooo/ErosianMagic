...
@AutoSpellConfig
public class MisleadSpell extends AbstractSpell {
    private final ResourceLocation spellId = new ResourceLocation(ErosianMagic.MODID, "mislead");
            ...
            multiTargetingData.getTargets().forEach(target -> {
                ...
                if(!Utils.shouldHealEntity(entity, targetEntity) && targetEntity != null) {
                    if(targetEntity instanceof Mob mob) {
                        ...
                        mob.setTarget(mainTarget);
                        mob.setLastHurtByMob(mainTarget);
                        mob.setLastHurtMob(mainTarget);
